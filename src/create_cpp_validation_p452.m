...
fprintf(fid, 'bool test%d() {\n', iname);
count = count + 1;
fprintf(fid,'std::string path = "./R-REC-P/"; // Path to the P452 maps DN50.TXT and N050.TXT\n');
fprintf(fid, 'P452DigitalMaps maps = P452DigitalMaps(path); // load P452 maps;\n');

fprintf(fid,'// Path profile: %s\n', filename1(1:end-4));
fprintf(fid, 'bool passed = true;\n');
...
fprintf(fid, 'double rDist[%d];\n', length(p452.path.d));
fprintf(fid, 'double rHeight[%d];\n', length(p452.path.d));
fprintf(fid, 'double g[%d];\n', length(p452.path.d));
fprintf(fid, 'int iZone[%d];\n', length(p452.path.d));


for ii = 1 : length(p452.path.d)
    fprintf(fid, 'rDist[%d] = %.9f;\n', ii-1, p452.path.d(ii));
end
for ii = 1 : length(p452.path.d)
...
fprintf(fid, 'double ff[%d];\n', nrows);
fprintf(fid, 'double pp[%d];\n', nrows);

for ii = 1 : nrows
    fprintf(fid, 'ff[%d] = %.16f;\n', ii-1, ff(ii));
end

for ii = 1 : nrows
    fprintf(fid, 'pp[%d] = %.16f;\n', ii-1, pp(ii));
end
...
fprintf(fid, 'double expectedResult[%d];\n',nrows);
 
for ii = 1:nrows
    fprintf(fid, 'expectedResult[%d] =  %.16f;\n', ii-1, Lb(ii));
end

fprintf(fid, 'vector<double> rDisti;\n');
fprintf(fid, 'rDisti.reserve(%d);\n', length(p452.path.d));
fprintf(fid, 'vector<double> rHeighti;\n');
fprintf(fid, 'rHeighti.reserve(%d);\n', length(p452.path.d));
fprintf(fid, 'vector<double> gHeighti;\n');
fprintf(fid, 'gHeighti.reserve(%d);\n', length(p452.path.d));
fprintf(fid, 'vector<int> iZonei;\n');
fprintf(fid, 'iZonei.reserve(%d);\n', length(p452.path.d));
fprintf(fid, 'for (int i = 0; i < %d; i++) {\n', length(p452.path.d));
fprintf(fid, '       rDisti.push_back(rDist[i]);\n');
fprintf(fid, '       rHeighti.push_back(rHeight[i]);\n');
fprintf(fid, '       gHeighti.push_back(g[i]);\n');
fprintf(fid, '       iZonei.push_back(iZone[i]);\n');
fprintf(fid, '}\n');
